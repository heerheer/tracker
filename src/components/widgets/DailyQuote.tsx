...
const DailyQuote: React.FC = () => {
    ...
    useEffect(() => {
        const api = isChinese() ? 'https://v1.hitokoto.cn' : 'https://thequoteshub.com/api/';
        fetch(api)
            .then(res => res.json())
            .then(data => {
    ...
    if (loading) {
        return (
            <section className="bg-[#FCFBFC] border border-[#DBDCD7] rounded-[28px] p-8 paper-shadow text-center flex flex-col items-center justify-center space-y-4 min-h-[140px]">
                ...
                <p className="text-[10px] uppercase tracking-widest text-[#726C62] font-medium opacity-50">Seeking wisdom...</p>
            </section>
        );
    }
